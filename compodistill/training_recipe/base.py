...
class BaseTrainingRecipe:
    ...
    def load(self, model, model_args={}):
        # a LoRA checkpoint is detected by its adapter_config.json
        if not os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json')): # loading model for non-lora/non-qlora pretraining
            model.load_llm(**model_args['llm'])
            model.load_vision_tower(**model_args['vision_tower'])
            model.load_connector(**model_args['connector'])
        else:
            model.language_model = model.language_model.from_pretrained(model_args['llm']['model_name_or_path'],attn_implementation=model_args['llm']['attn_implementation'],torch_dtype=model_args['llm']['torch_dtype'])
            model.load_vision_tower(**model_args['vision_tower'])
            model.load_connector(**model_args['connector'])
            ...
            if model_args['pretrain_param_merge']:
                model = model.merge_and_unload()
                print("NOTICE : Model is Merge and Unload.")
