...
# DATASET_PATH
DISTILL_DATA_PATH=CHANGE_TO_YOUR_DATA_DIR/blip_laion_cc_sbu_558k.json 
DISTILL_IMAGE_PATH=CHANGE_TO_YOUR_DATA_DIR/images/

DISTILL_TRAIN_RECIPE=common
...
TEACHER_CONV_VERSION=llama
TEACHER_VERSION=base-lora-zero2-r128

TEACHER_VT_VARIANT="${TEACHER_VT_VERSION#*/}"
TEACHER_LLM_VARIANT="${TEACHER_LLM_VERSION#*/}"

OUTPUT_FILE_NAME="CompoDistill-${STUDENT_LLM_VARIANT}-${STUDENT_VT_VARIANT}-${STUDENT_VERSION}-distill_pretrain"

OUTPUT_FILE_NAME="${OUTPUT_FILE_NAME}-post_cn"
...
args=(
    ...
    --pretrained_model_path         ./checkpoints/CompoDistill-${STUDENT_LLM_VARIANT}-${STUDENT_VT_VARIANT}-${STUDENT_VERSION}-pretrain
...
deepspeed --include localhost:0,5,6,7 compodistill/train/train_dpt.py "${args[@]}"