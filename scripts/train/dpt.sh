...
#
# The student's connector is replaced by the teacher's (frozen) connector and only the
# post-connector (teacher hidden -> student hidden) is trained with the LM + logit
# distillation loss on the LLaVA pre-training data.
#
# Requires: student pretrain checkpoint (pretrain.sh) and teacher checkpoint (finetune.sh,
# or the merged teacher released on HuggingFace).
# Effective batch size = 8 (per device) x 8 (grad accum) x 4 (GPUs) = 256.
...
OUTPUT_NAME="CompoDistill-${STUDENT_LLM_VARIANT}-SigLIP-DPT"
...
deepspeed --include localhost:0,1,2,3 --master_port 29503 compodistill/train/train.py "${args[@]}"
